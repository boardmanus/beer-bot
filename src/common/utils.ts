class Utils {
  static f_to_c(f: number): number {
    return ((f - 32.0) * 5.0) / 9.0;
  }

  static c_to_f(c: number): number {
  ...
  static gravity_to_abv(og: number, g: number): number {
    return (og - g) * 131.25;
  }

  static rgba_style(rgb: [number, number, number], a: number): string {
    return `rgba(${rgb[0]},${rgb[1]},${rgb[2]},${a})`;
  }

  static srm_to_rgb(srm: number): [number, number, number] {
