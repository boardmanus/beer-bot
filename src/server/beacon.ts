  ...
  startScan(): Promise<unknown> {
    const promise = new Promise((resolve: (value: void) => void) => {
      this.timer = setInterval(() => {
        ...
        this.minor = Math.round(1000.0 * this.reading.gravity + Utils.noise(NOISE_GRAVITY));
        this.major = Math.round(Utils.c_to_f(this.reading.temperature + Utils.noise(NOISE_TEMPERATURE)));
...
class Beacon {
  ...
  constructor(onAdvertisement: (details: TiltPayload) => void) {
    ...
    this.beaconScanner.onadvertisement = (details) => onAdvertisement(toTiltPayload(details));
